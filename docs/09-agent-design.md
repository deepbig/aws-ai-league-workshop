...
확정 규칙 반영: **보물 도달=게임종료**(코인 최대 수집 후 마지막에 보물 — `max_loot` 전략),
**남은 생명=점수**(장애물 회피), **막힘 강제통과=게임종료**(경로 유효성 최우선),
**토큰 보너스**(챌린지당 토큰 절약), **챌린지 +코인/오답 ♥-1**(저신뢰 SKIP), **LLM-as-judge 채점**.
**실격 방지**: 도구 내 외부 모델 금지 · 프롬프트 정답 하드코딩 금지 · 범위 외 작업 금지. 맵 아이템은 mapId(c1..cN).
